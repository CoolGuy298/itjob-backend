...
const isAdminOrOwner = (user, resourceOwnerId) => {
    return user.role === 'admin' || user.role === 'superadmin' || user.id === resourceOwnerId.toString();
};
...
exports.deleteCompany = async (req, res) => {
    ...
    try {
        ...
        if (!isAdminOrOwner(req.user, companyId)) {
...
exports.updateCompanyVisibility = async(req,res) =>{
        ...
        try {
            const company = await Company.findByIdAndUpdate(id, { isVisible }, { new: true });
            if (!company) {
                return res.status(404).json({ message: 'Comapny not found' });
            }
            res.status(200).json({ message: 'Company visibility updated successfully', company });
        } catch (error) {
            res.status(500).json({ message: 'Error updating company visibility', error });
        }
      }